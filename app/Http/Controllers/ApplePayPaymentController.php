...
class ApplePayPaymentController extends Controller
{
     ...
     */
    public function applePay(Request $request): View
    {
        $order_id = $request->query('order_id');
        return view('apple-pay', compact('order_id'));
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function applePayPost(Request $request, $order_id): RedirectResponse
    {
        try {
            $data = ApplePayOrder::where('order_id', $order_id)->first();

            if (!$data) {
                return redirect()->back()->with('error', 'Order not found');
            }

            // Verify Apple Pay payment was successful
            // In a real implementation, you would verify the Apple Pay transaction here
            // For now, we'll assume the payment was successful since Apple Pay redirected here
...
foreach($carts as $cart){
    try {
        ...
        return redirect($redirectUrl);
            ...
            \Log::error('Apple Pay payment processing failed for order ' . $order_id . ': ' . $e->getMessage(), [
                'exception' => $e,
